...
def main() -> None:
    ...
    print(f"option pills (key+pills) : {result.option_rows:>6}  "
          f"(key-cap={result.choice_key_hit}, >={CONFIG.option_veto_count} vetoes) "
          f"-> many_options={result.many_options}")
    print("-" * 48)
    ...
    for roi, color, label in (
        ...
        (CONFIG.choice_key_roi, (0, 255, 255), "key"),
    ):
